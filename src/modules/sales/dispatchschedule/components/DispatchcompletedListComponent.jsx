...
class DispatchcompletedListComponent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  render() {
    ...
    let table = (
      <div>
        {!this.state.dispatchListData || !this.state.dispatchListData.length
          ? (
            <Col xs={12}>No dispatch complete list found. </Col>
          ) : (
    ...
    let addEditDispatchModal = (
      <DispatchModalComponent
        ...
        dispatchSchedule={this.state.dispatch}
        {...this.props}
      />
    );
    return (
...
export default DispatchcompletedListComponent;