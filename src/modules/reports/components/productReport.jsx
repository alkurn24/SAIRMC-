...
import { getProductReport } from "../server/ReportsServerComm"
    ...
    if (_this.state.plantList.length) {
      ...
      getProductReport(params,
        function success(data) {
          ...
          let tempData = data.Report.map((prop, key) => {
            if (key === 0) {
              prop.products.map(product => {
                columns.push({ Header: product.name, accessor: product.name })
                return true
              });
            }
            ...
            prop.products.map((product) => {
              <div style={{ textAlign: "right" }}>
                {row[product.name] = product.total.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
              </div>
              return true
