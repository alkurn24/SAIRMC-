...
import { getInventoryInwardReport } from "../server/ReportsServerComm"
...
class RawMaterialInwardReportComponent extends Component {
	constructor(props) {
		super(props);
		this.state = {
			data: [],
	...
	search(state) {
		...
		if (_this.state.plantList.length) {
			...
			getInventoryInwardReport(params,
				function success(data) {
					console.log(data);
					var columns = [
					...
					let tempData = data.StoreInward.map((prop, key) => {
						if (key === 0) {
							prop.stores.map(store => {
								columns.push({ Header: store.name, accessor: store.name })
								return true
							});
						}
						...
						prop.stores.map((store) => {
							row[store.name] = store.total.toLocaleString("en-IN", { minimumFractionDigits: 2 })
							return true
						})
						return row;
					})
	...
	JSONToCSVConvertor(JSONData, ReportTitle, ShowLabel) {
		...
		var fileName = ("Raw_Material_Inward_Report_");
...
export default RawMaterialInwardReportComponent;