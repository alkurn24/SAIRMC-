...
class InwardListComponent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  fetchDataFromServer(state) {
    let _this = this;
    let params = "view=movement&movementType=Inward";
      ...
      <Col xs={12}>
        {
          !this.state.movementList || !this.state.movementList.length
            ? (
              <div>No inward list found. <a href="#/movements/movement-edit/new">Click here</a> to create one.</div>
            ) : (
              <div>
                <Row>
                  <Col xs={12}>
        ...
        <Col xs={12} sm={6} md={3} lg={3}>
          <FormGroup>
            <FormControl
              type="text"
              name="movementType"
              placeholder="Search by mpvement type"
...
export default InwardListComponent;